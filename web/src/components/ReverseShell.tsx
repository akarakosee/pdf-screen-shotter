...
export function ReverseShell({ t = en, desktopAppUrl }: Props) {
  ...
  const controller = useCallback((): JobController => {
    if (!controllerRef.current) {
      controllerRef.current = new JobController({
        ...
        onReverseProgress: (processed, total) => {
           setProgress({ message: `Reversing page ${processed} of ${total}...`, percentage: (processed / total) * 100 });
        },
        onDone: (res) => {
          setResult(res);
          setCancelling(false);
