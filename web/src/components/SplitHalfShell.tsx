...
export function SplitHalfShell({ t = en, desktopAppUrl }: Props) {
    ...
    if (!controllerRef.current) {
      controllerRef.current = new JobController({
        onSplitHalfProgress: (processed, total) => {
           setProgress({ message: `Splitting page ${processed} of ${total}...`, percentage: (processed / total) * 100 });
        },
        onSplitHalfDone: (res) => {
          setResult(res);
          setCancelling(false);
    ...
    <div className="w-full flex flex-col gap-5">
      ...
      {phase === 'done' && result && (
        <ResultPanel
          ...
          crossLink={null}
                    onDownload={() => {
            if (result.output) triggerDownload(result.output, result.outputName || 'split-half.pdf');
          }}
          onConvertMore={reset}
        />
      )}
