...
export function RemoveBlankShell({ t = en, desktopAppUrl }: Props) {
    ...
    if (!controllerRef.current) {
      controllerRef.current = new JobController({
        ...
        onRemoveBlankProgress: (processed, total) => {
           setProgress({ message: `Analyzing page ${processed} of ${total}...`, percentage: (processed / total) * 100 });
        },
        onDone: (res) => {
          setResult(res);
          setCancelling(false);
  ...
  const addFiles = useCallback(
    (files: File[]) => {
      ...
      const file = files[0];
      setPhase('processing');
      controller().runRemoveBlank(file);
    },
    [controller]
  );
