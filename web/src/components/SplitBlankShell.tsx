...
export function SplitBlankShell({ t = en }: Props) {
  ...
  const addFile = useCallback(async (incoming: File[]) => {
    ...
    setFile(f);
    setPhase('processing');
    controller.current?.runSplitBlank(f);
  }, [t]);
  ...
  return (
    <div className="flex flex-col gap-5">
      ...
      {phase === 'processing' && (
        <ProgressPanel label={t.lang === 'tr' ? 'Boş sayfalar tespit ediliyor ve belgeler bölünüyor...' : 'Detecting blank separator pages and splitting documents...'} />
      )}
