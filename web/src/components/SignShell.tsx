...
export function SignShell({ t = en }: Props) {
  ...
  useEffect(() => {
    ...
    let active = true;
    file.arrayBuffer().then((buf) => PDFDocument.load(buf, { ignoreEncryption: true })).then((doc) => {
      if (active) {
        const count = doc.getPageCount();
        setTotalPages(count);
        setPreviewPageNum(count); // Default to last page
      }
    }).catch(() => {});
    return () => { active = false; };
  }, [file]);
          ...
          <div className="flex flex-col items-center justify-center text-center gap-4 py-2">
            ...
            <div className="flex flex-col gap-1">
              <h3 className="text-lg font-semibold">
                {t.lang === 'tr' ? 'PDF Başarıyla İmzalandı!' : 'PDF Successfully Signed!'}
              </h3>
              <p className="text-sm text-ink-muted dark:text-ink-muted-dark">
                {t.lang === 'tr'
                  ? `Belgenize imza yerleştirildi (${output.pagesSigned} sayfa) ve indirmeye hazır.`
                  : `Signature applied successfully (${output.pagesSigned} page${output.pagesSigned > 1 ? 's' : ''}).`}
              </p>
            </div>
