...
function getPageNumberText(mode: NumberStyle, pageNum: number, totalPages: number, lang?: 'tr' | 'en'): string {
  if (mode === 'roman') {
    return `${toRoman(pageNum)}`;
  }
  if (mode === 'roman-slash') {
    return `${toRoman(pageNum)} / ${toRoman(totalPages)}`;
  }
  if (lang === 'tr') {
    switch (mode) {
      case 'simple': return `${pageNum}`;
      case 'prefix': return `Sayfa ${pageNum}`;
      case 'slash': return `${pageNum} / ${totalPages}`;
      case 'full': return `Sayfa ${pageNum} / ${totalPages}`;
      default: return `${pageNum}`;
    }
  } else {
    switch (mode) {
      case 'simple': return `${pageNum}`;
      case 'prefix': return `Page ${pageNum}`;
      case 'slash': return `${pageNum} of ${totalPages}`;
      case 'full': return `Page ${pageNum} of ${totalPages}`;
      default: return `${pageNum}`;
    }
  }
}
          ...
          <div className="flex flex-col items-center justify-center text-center gap-4 py-2">
            ...
            <div className="flex flex-col gap-1">
              <h3 className="text-lg font-semibold">
                {t.lang === 'tr' ? 'Sayfa Numaraları Başarıyla Eklendi!' : 'Page Numbers Successfully Added!'}
              </h3>
              <p className="text-sm text-ink-muted dark:text-ink-muted-dark">
                {t.lang === 'tr' ? 'Belgenize sayfa numaraları işlendi ve indirmeye hazır.' : 'Page numbers have been applied to your PDF.'}
              </p>
            </div>
            ...
            <div className="flex flex-wrap items-center justify-center gap-3 pt-2">
              <Button variant="ghost" onClick={reset} className="flex items-center gap-2">
                ...
                {t.lang === 'tr' ? 'Yeni Dosya Seç' : 'Number Another PDF'}
              </Button>
