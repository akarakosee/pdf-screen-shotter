...
export function ExtractJavascriptShell({ t = en }: Props) {
  ...
  useEffect(() => {
    ...
    setPhase('upload');
      ...
      onExtractJavascriptDone: (result) => {
    ...
    const errMsg = t.lang === 'tr' 
              ? 'Bu PDF dosyasında gömülü herhangi bir JavaScript kodu bulunamadı (Temiz Belge).' 
              : 'No embedded JavaScript found in this document (Clean Document).';
          setErrorMsg(errMsg);
  ...
  const addFile = useCallback(async (incoming: File[]) => {
    ...
    setFile(f);
    setPhase('processing');
    controller.current?.runExtractJavascript(f);
  }, [t]);
  ...
  return (
    <div className="flex flex-col gap-5">
      ...
      {phase === 'processing' && (
        <ProgressPanel label={t.lang === 'tr' ? 'Gömülü JavaScript kodları ve tehditler taranıyor...' : 'Scanning for embedded JavaScript & threat payloads...'} />
      )}
