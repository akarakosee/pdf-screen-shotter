...
export function SmartMarkdownShell({ t = en }: Props) {
  ...
  const addFile = useCallback(async (incoming: File[]) => {
    ...
    setFile(f);
    setPhase('processing');
    controller.current?.runSmartMarkdown(f);
  }, [t]);
  ...
  return (
    <div className="flex flex-col gap-5">
      ...
      {phase === 'processing' && (
        <ProgressPanel label={t.lang === 'tr' ? 'PDF yapay zeka ile akıllı Markdown (.md) biçimine dönüştürülüyor...' : 'Converting PDF to Smart Markdown...'} />
      )}
