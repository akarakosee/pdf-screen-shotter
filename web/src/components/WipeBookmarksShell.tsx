...
export function WipeBookmarksShell({ t = en }: Props) {
  ...
  useEffect(() => {
    controller.current = new JobController({
      ...
      onWipeBookmarksDone: (result) => {
        if (result.output && result.succeeded > 0) {
          setOutput({ blob: result.output, name: result.outputName! });
          setPhase('done');
        } else {
          setPhase('upload');
          setToast({ kind: 'error', message: t.lang === 'tr' ? 'Yer imleri silinirken bir hata oluştu.' : 'Error wiping bookmarks.' });
        }
      }
    });
    return () => {
  ...
  const addFile = useCallback(async (incoming: File[]) => {
    ...
    setFile(f);
    setPhase('processing');
    controller.current?.runWipeBookmarks(f);
  }, [t]);
      ...
      {phase === 'processing' && (
        <div className="phase-enter flex flex-col gap-3">
          <div className="flex items-baseline justify-between text-xs text-ink-muted dark:text-ink-muted-dark">
            <span>{t.lang === 'tr' ? 'Yer imleri ve içindekiler ağacı temizleniyor...' : 'Wiping bookmarks and outlines tree...'}</span>
          </div>
