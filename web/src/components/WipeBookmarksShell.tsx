...
export function WipeBookmarksShell({ t = en }: Props) {
    ...
    setPhase('upload');
      ...
      onWipeBookmarksDone: (result) => {
        if (result.output && result.succeeded > 0) {
          ...
          setErrorMsg(null);
    const errMsg = t.lang === 'tr' ? 'Yer imleri silinirken bir hata oluştu.' : 'Error wiping bookmarks.';
          setErrorMsg(errMsg);
  ...
  return (
    <div className="flex flex-col gap-5">
      ...
      {phase === 'processing' && (
        <ProgressPanel label={t.lang === 'tr' ? 'Yer imleri ve içindekiler ağacı temizleniyor...' : 'Wiping bookmarks and outlines tree...'} />
      )}
