...
export function ExtractTocShell({ t = en }: Props) {
  ...
  const addFile = useCallback(async (incoming: File[]) => {
    ...
    setFile(f);
    setPhase('processing');
    controller.current?.runExtractToc(f);
  }, [t]);
  ...
  return (
    <div className="flex flex-col gap-5">
      ...
      {phase === 'processing' && (
        <ProgressPanel label={t.lang === 'tr' ? 'PDF içindekiler ve yer imleri yapısı taranıyor...' : 'Scanning PDF Table of Contents and Bookmarks...'} />
      )}
