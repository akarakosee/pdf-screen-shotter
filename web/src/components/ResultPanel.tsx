    ...
    >
      <div className="flex flex-col items-center justify-center text-center gap-4 py-2">
        <div className={`flex h-12 w-12 items-center justify-center rounded-full ${isError ? 'bg-danger/10 text-danger' : 'bg-success/10 text-success'}`}>
          {isError ? <AlertCircle className="h-6 w-6" /> : <Check className="h-6 w-6" />}
        </div>
        <div className="flex flex-col gap-1">
          <h3 className="text-lg font-semibold text-ink dark:text-ink-dark">
            {isError 
              ? (t.lang === 'tr' ? 'İşlem Başarısız Oldu' : 'Process Failed') 
        ...
        {isError ? (
          <div className="flex flex-wrap items-center justify-center gap-3 pt-2">
            <Button variant="primary" className="result-pop flex items-center gap-2" onClick={onConvertMore}>
