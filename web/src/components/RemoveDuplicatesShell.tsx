...
export function RemoveDuplicatesShell({ t = en }: Props) {
  ...
  const addFile = useCallback(async (incoming: File[]) => {
    ...
    setFile(f);
    setPhase('processing');
    controller.current?.runRemoveDuplicates(f);
  }, [t]);
  ...
  return (
    <div className="flex flex-col gap-5">
      ...
      {phase === 'processing' && (
        <ProgressPanel label={t.lang === 'tr' ? 'Mükerrer ve yinelenen sayfalar taranıyor ve ayıklanıyor...' : 'Scanning and removing duplicate pages...'} />
      )}
