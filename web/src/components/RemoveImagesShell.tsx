...
export function RemoveImagesShell({ t = en }: Props) {
  ...
  const addFile = useCallback(async (incoming: File[]) => {
    ...
    setFile(f);
    setPhase('processing');
    controller.current?.runRemoveImages(f);
  }, [t]);
  ...
  return (
    <div className="flex flex-col gap-5">
      ...
      {phase === 'processing' && (
        <ProgressPanel label={t.lang === 'tr' ? 'Tüm resim ve fotoğraflar siliniyor, metinler korunuyor...' : 'Removing all images while preserving text and layout...'} />
      )}
