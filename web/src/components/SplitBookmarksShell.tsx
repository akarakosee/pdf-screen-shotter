...
export function SplitBookmarksShell({ t = en }: Props) {
  ...
  useEffect(() => {
    ...
    const errMsg = t.lang === 'tr' 
              ? 'Bu PDF belgesinde bölümlere ayırmak için herhangi bir Yer İmi (Bookmarks / İçindekiler) bulunamadı.' 
              : 'Could not find any Table of Contents (Bookmarks) to split by.';
          setErrorMsg(errMsg);
  ...
  const addFile = useCallback(async (incoming: File[]) => {
    ...
    setFile(f);
    setPhase('processing');
    controller.current?.runSplitBookmarks(f);
  }, [t]);
  ...
  return (
    <div className="flex flex-col gap-5">
      ...
      {phase === 'processing' && (
        <ProgressPanel label={t.lang === 'tr' ? 'Yer imlerine göre bölümler ayrıştırılıyor...' : 'Splitting chapters by bookmarks...'} />
      )}
