...
export function RepairShell({ t = en, desktopAppUrl }: Props) {
  ...
  const controller = useCallback((): JobController => {
    if (!controllerRef.current) {
      controllerRef.current = new JobController({
        ...
        onFatal: () => {
          setCancelling(false);
          setToast({ kind: 'error', message: t.corruptFile || 'Could not repair file.' });
          setErrorMsg(null);
    setPhase('upload');
        },
      });
  ...
  const addFiles = useCallback(
    (files: File[]) => {
      ...
      const file = files[0];
      setPhase('processing');
      // We will add runRepair to JobController
      (controller() as any).runRepair(file);
    },
    [controller]
  );
  ...
  return (
    <div className="flex flex-col gap-5">
      ...
      {phase === 'processing' && (
        <ProgressPanel
          label={t.converting || 'Repairing PDF...'}
          progressPercent={
            progress
      ...
      {phase === 'done' && (result || errorMsg) && (
        <div className="animate-in fade-in slide-in-from-bottom-8 flex flex-col items-center justify-center py-8 duration-700 w-full mx-auto">
          <ResultPanel
            ...
            crossLink={null}
            onDownload={() => {
              if (result.output) triggerDownload(result.output, result.outputName || 'repaired.pdf');
            }}
            onConvertMore={reset}
          />
        </div>
