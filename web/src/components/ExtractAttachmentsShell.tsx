...
export function ExtractAttachmentsShell({ t = en }: Props) {
  ...
  const addFile = useCallback(async (incoming: File[]) => {
    ...
    setFile(f);
    setPhase('processing');
    controller.current?.runExtractAttachments(f);
  }, [t]);
  ...
  return (
    <div className="flex flex-col gap-5">
      ...
      {phase === 'processing' && (
        <ProgressPanel label={t.lang === 'tr' ? 'Gömülü veri ve ek dosyalar çıkarılıyor...' : 'Extracting embedded file attachments...'} />
      )}
