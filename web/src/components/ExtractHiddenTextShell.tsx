...
export function ExtractHiddenTextShell({ t = en }: Props) {
    ...
    setPhase('upload');
      ...
      onExtractHiddenTextDone: (result) => {
        if (result.output && result.succeeded > 0) {
          ...
          setErrorMsg(null);
    const errMsg = t.lang === 'tr' ? 'Belgede gizli veya okunabilir metin katmanı bulunamadı.' : 'No text layers found in this document.';
          setErrorMsg(errMsg);
  ...
  return (
    <div className="flex flex-col gap-5">
      ...
      {phase === 'processing' && (
        <ProgressPanel label={t.lang === 'tr' ? 'Gizli metin ve katmanlar taranıyor...' : 'Scanning hidden text & forensic layers...'} />
      )}
