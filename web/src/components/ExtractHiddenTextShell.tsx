...
export function ExtractHiddenTextShell({ t = en }: Props) {
    ...
    controller.current = new JobController({
      ...
      onExtractHiddenTextDone: (result) => {
        if (result.output && result.succeeded > 0) {
          ...
          setPhase('upload');
          setToast({ kind: 'error', message: t.lang === 'tr' ? 'Belgede gizli veya okunabilir metin katmanı bulunamadı.' : 'No text layers found in this document.' });
        }
      }
    });
    return () => {
  ...
  const addFile = useCallback(async (incoming: File[]) => {
    ...
    setFile(f);
    setPhase('processing');
    controller.current?.runExtractHiddenText(f);
  }, [t]);
      ...
      {phase === 'processing' && (
        <div className="phase-enter flex flex-col gap-3">
          <div className="flex items-baseline justify-between text-xs text-ink-muted dark:text-ink-muted-dark">
            <span>{t.lang === 'tr' ? 'Gizli metin ve katmanlar taranıyor...' : 'Scanning hidden text & forensic layers...'}</span>
          </div>
