...
export function ExtractUrlsShell({ t = en }: Props) {
  ...
  const addFile = useCallback(async (incoming: File[]) => {
    ...
    setFile(f);
    setPhase('processing');
    controller.current?.runExtractUrls(f);
  }, [t]);
  ...
  return (
    <div className="flex flex-col gap-5">
      ...
      {phase === 'processing' && (
        <ProgressPanel label={t.lang === 'tr' ? 'Belgedeki tüm bağlantılar ve URL\'ler taranıyor...' : 'Scanning all links, hyperlinks & URLs...'} />
      )}
